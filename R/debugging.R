...
function_var_to_env = function(
  ...
  list_start = str_locate(called_fun_txt, "\\(")[1]
  ...
  arg_txt = capture.output(eval(parse(text = paste0("args(",fun_name, ")"))))
...
#' @section Side effects:
#' \itemize{
#'   \item runs the first line of function_1 up to the first useage of function_2 in \code{to_env}
#'   \item runs \code{\link{function_var_to_env}} on functnio 2 in  \code{to_env}
#'}
#'
#' @section Limitations:
#' \itemize{
...
function_to_function = function(
  ...
  fun_text = capture.output(eval(parse(text = fun_name_1)))
  
  fun_text = trimws(fun_text)
  fun_text = sapply(fun_text, function(each_line){
    comment_loci = str_locate(each_line, "\\#")[1]
  ...
  bracket_loci = str_locate(fun_text[1], "\\{")[[1]][1]
